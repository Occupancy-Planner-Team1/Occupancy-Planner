...
const SuggestedReservation = () => {
  ...
  const Indicator = useRef(useContext(AuthContext));
  ...
  async function getRecommendedTimeslot() {
    var timeslots = [];
    console.log(userid);
    await axios.get("/api/auth/res-all/user/" + userid, { headers: { Authorization: 'Bearer ' + sessionStorage.getItem('kc_token') } })
      .then(response => {
    ...
    console.log(timeSlots[mostCommonElement]);
